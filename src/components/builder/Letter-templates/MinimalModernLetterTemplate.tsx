...
export default function MinimalModernLetterTemplate({ 
  ...
  return (
    <div className={containerClass} style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {/* Clean Letterhead */}
      <header className={`${preview ? 'px-8 py-6' : 'px-16 py-12'} border-b border-gray-100`}>
        <div className="flex justify-between items-start">
          {/* Sender Info */}
          <div>
            <h1 className={`${preview ? 'text-xl' : 'text-2xl'} font-semibold text-gray-900 ${preview ? 'mb-4' : 'mb-6'} tracking-tight`}>
              {data.senderInfo.name}
            </h1>
            <div className={`space-y-2 ${preview ? 'text-sm' : 'text-sm'} text-gray-600`}>
          ...
          <div className="text-right">
            <div className={`${preview ? 'text-sm' : 'text-sm'} text-gray-600`}>{data.letterInfo.date}</div>
          </div>
        </div>
      </header>
      ...
      <main className={`${preview ? 'px-8 py-8' : 'px-16 py-12'}`}>
        ...
        <div className={`${preview ? 'mb-6' : 'mb-8'}`}>
          <div className={`${preview ? 'text-sm' : 'text-sm'} font-medium text-gray-500 uppercase tracking-wider mb-2`}>Subject</div>
          <h2 className={`${preview ? 'text-lg' : 'text-xl'} font-medium text-gray-900`}>{data.letterInfo.subject}</h2>
        </div>
        ...
        <div className={`${preview ? 'mb-8' : 'mb-12'}`}>
          <div className={`prose ${preview ? 'prose-base' : 'prose-lg'} max-w-none`}>
            {data.letterInfo.body.split('\n\n').map((paragraph: string, index: number) => (
              <p key={index} className={`text-gray-700 leading-relaxed ${preview ? 'mb-4' : 'mb-6'} text-justify`}>
                {paragraph}
              </p>
            ))}
          </div>
